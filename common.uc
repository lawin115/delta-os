...
function __phy_is_fullmac(phyidx)
{
	...
	if (!data || !data.software_iftypes) return true; return !data.software_iftypes.monitor;
}
...
function wdev_create(phy, name, data)
{
	...
	if (!reuse_ifname) {
		system(`iw phy ${phy} interface add ${name} type managed >/dev/null 2>&1`);
		reuse_ifname = name;
	}

	if (reuse_ifname) {
		if (reuse_ifname != name) {
			rtnl.request(rtnl.const.RTM_SETLINK, 0, { dev: reuse_ifname, ifname: name});
		}
		nl80211.request(
